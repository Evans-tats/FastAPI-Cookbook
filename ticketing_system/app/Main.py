...
from .operations import create_ticket, create_event, get_ticket, update_price_of_ticket,delete_ticket
# get_ticket, update_price_of_ticket, delete_ticket
...
async def create_ticket_route(ticket: TicketCreate,db_session: AsyncSession = Depends(get_db_session)):
    ticket = await create_ticket(ticket,db_session)
    return TicketRead.model_validate(ticket)
@app.post("/event", status_code=status.HTTP_201_CREATED)
async def create_event_route(event : EventCreate, db_seesion : AsyncSession = Depends(get_db_session)):
    event = await create_event(event, db_seesion)
